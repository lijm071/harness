...
```mermaid
flowchart TD
    User["用户 / CLI"] --> CLI["coding_agent.cli<br/>参数解析与启动装配"]
    CLI --> Workspace["WorkspaceContext<br/>工作区快照"]
    CLI --> Model["Model Client<br/>Ollama / OpenAI / Anthropic"]
    CLI --> Runtime["Agent Runtime<br/>控制循环"]

    Runtime --> Context["ContextManager<br/>prompt 预算与上下文压缩"]
    Runtime --> Memory["LayeredMemory<br/>工作记忆 / 文件摘要 / 持久笔记"]
    Runtime --> Tools["Tool Registry<br/>受控工具白名单"]
    Runtime --> Store["RunStore & SessionStore<br/>会话与运行产物"]

    Context --> Model
    Model --> Runtime
    Runtime --> Parser["响应解析<br/><tool> / <final>"]
    Parser --> Tools

    Tools --> FS["Workspace 文件系统"]
    Tools --> Shell["Shell 命令"]
    Tools --> Delegate["只读子 Agent"]

    Store --> Artifacts[".coding-agent/sessions<br/>.coding-agent/runs"]
```
