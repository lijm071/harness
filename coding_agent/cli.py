...
这个模块负责把“用户怎么启动 Coding Agent”翻译成 runtime 能理解的对象：
...
WELCOME_NAME = "coding-agent"
WELCOME_SUBTITLE = "repository automation runtime"
...
LEGACY_SECRET_ENV_NAMES_VAR = "CODING_AGENT_LEGACY_SECRET_ENV_NAMES"
SECRET_ENV_NAMES_VAR = "CODING_AGENT_SECRET_ENV_NAMES"
...
def build_agent(args):
    """根据 CLI 参数装配出一个可运行的 CodingAgent 实例。
    ...
    - 输出：一个新的 `CodingAgent`，或一个从旧 session 恢复出来的 `CodingAgent`
    ...
    # 还是创建一个新的 CodingAgent 实例。
    configured_secret_names = _configured_secret_names(args)
    workspace = WorkspaceContext.build(args.cwd)
    store = SessionStore(workspace.repo_root + "/.coding-agent/sessions")
    model = _build_model_client(args)
    session_id = args.resume
    if session_id == "latest":
        session_id = store.latest()
    if session_id:
        return CodingAgent.from_session(
            model_client=model,
            workspace=workspace,
            session_store=store,
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        ...
        description="Repository automation runtime for Ollama, OpenAI-compatible, or Anthropic-compatible models.",
    )
