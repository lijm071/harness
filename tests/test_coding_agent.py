...
import coding_agent as agent_pkg
...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".coding-agent" / "sessions")
...
def test_agent_only_stores_reusable_epistemic_notes(tmp_path):
    ...
    resumed = CodingAgent.from_session(
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".coding-agent").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".coding-agent" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "coding" / "agent" / "welcome" / "screen"
    ...
    assert "coding-agent" in welcome
    assert "repository automation runtime" in welcome
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "coding_agent.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("coding_agent.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = agent_pkg.build_agent(args)
...
def test_build_arg_parser_defaults_provider_to_openai(tmp_path):
    args = agent_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider == "openai"


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = agent_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "coding_agent.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("coding_agent.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("coding_agent.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            agent_pkg.build_agent(args)
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .coding-agent/.\n"
    ...
    index_path = tmp_path / ".coding-agent" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".coding-agent" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".coding-agent" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .coding-agent/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".coding-agent" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".coding-agent" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(agent_pkg.__file__).as_posix().endswith("/coding_agent/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(agent_pkg.main)
    assert callable(agent_pkg.build_agent)
    assert callable(agent_pkg.build_arg_parser)
