...
from coding_agent import FakeModelClient, CodingAgent, SessionStore, WorkspaceContext
from coding_agent import cli as agent_cli
from coding_agent.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = agent_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = agent_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = agent_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = agent_cli.build_agent(args)
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "CODING_AGENT_LEGACY_SECRET_ENV_NAMES": "MCA_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("coding_agent.cli.OllamaModelClient", DummyModelClient):
        args = agent_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = agent_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("coding_agent.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "coding_agent.runtime"

    with patch("coding_agent.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
